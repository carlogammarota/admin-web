<template>
    <div>
            <!-- pop up Delete -->
    <div  v-if="modalCancel" id="popup-modal" tabindex="-1" class="bg-black fixed top-0 left-0 right-0 z-50 p-4 overflow-y-auto md:inset-0 h-modal md:h-full h-screen w-full">
        <div class="relative w-full h-full max-w-md md:h-auto m-auto mt-24">
            <div class="relative bg-gray-100 rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    ...
                    <button data-modal-hide="popup-modal" v-if="modalCancel = false" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancel</button>
                </div>
            </div>
        </div>
    </div>
      ...
      <tr class="hover:bg-gray-50" v-for="item in blogs" :key="item.id">
        ...
        <td class="px-6 py-4" >
          <div class="flex justify-end gap-4">
            <button @click="ConfirmDeleteItem(item._id)">
...
export default {
    ...
    methods: {
        ...
        deleteItem(_id){
            // console.log("id", id);
            feathers.default.blog.remove(_id).then(data => {
                this.getBlogs();
